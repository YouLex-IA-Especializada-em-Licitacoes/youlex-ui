...
```bash
npx shadcn add https://youlex-ia-especializada-em-licitacoes.github.io/youlex-ui/approval-card.json
```

Or, once a consumer repo declares the `@youlex` entry in its `components.json`
(see `registries`), simply:

```bash
npx shadcn add @youlex/approval-card
```

Browse the index at
[`registry.json`](https://youlex-ia-especializada-em-licitacoes.github.io/youlex-ui/registry.json).
Each item declares its `registryDependencies`, npm `dependencies`, and `files`, so one
...
`public/r/*.json` is generated by `bun run registry` (`scripts/build-registry.mjs`) and published
by [`.github/workflows/publish-registry.yml`](.github/workflows/publish-registry.yml) to GitHub
Pages on every push to `develop`. The site root **is** `public/r/` — there is no `/r/` segment in
the served URL.

### What happens when a component changes here

`shadcn add` **copies** the component's files into the consumer repo at install time — it does
not create a live link back to this registry. Once a team pulls a component into
`YOULEX-FRONTEND`, that copy stays exactly as it is when `develop` here moves on; nobody is
silently broken by an upstream change mid-sprint.

To pick up a later version, re-run the same command with `--overwrite`:

```bash
npx shadcn add @youlex/approval-card --overwrite
```

This is a deliberate choice, not an accident of the tool: automatic re-pull on every deploy would
mean a design-system change could change frontend behavior without a frontend PR, code review, or
changelog entry. Explicit `--overwrite` keeps every component update visible as a normal commit in
the consumer repo.
